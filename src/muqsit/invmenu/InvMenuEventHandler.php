		...
		if(count($packets) === 1) {
			...
			if($packet instanceof ContainerOpenPacket) {
				...
				if(count($targets) === 1) {
					...
					if($session !== null && $session->getNetwork()->translateContainerOpen($session, $packet->windowId, $packet->windowType, $packet->blockPosition->getX(), $packet->blockPosition->getY(), $packet->blockPosition->getZ())) {
						$event->cancel();
					}
				}
			}
