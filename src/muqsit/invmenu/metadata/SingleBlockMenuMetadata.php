...
class SingleBlockMenuMetadata extends MenuMetadata {
	...
	protected function sendGraphicAt(Vector3 $pos, Player $player, MenuExtradata $metadata): void {
		$player->getNetworkSession()->sendDataPacket(UpdateBlockPacket::create(new BlockPosition($pos->x, $pos->y, $pos->z), RuntimeBlockMapping::getInstance()->toRuntimeId($this->block->getFullId()), UpdateBlockPacket::FLAG_NETWORK, UpdateBlockPacket::DATA_LAYER_NORMAL));
	}
	...
	public function removeGraphic(Player $player, MenuExtradata $extradata): void {
		...
		foreach($this->getBlockPositions($extradata) as $position) {
			...
			$network->sendDataPacket(UpdateBlockPacket::create(new BlockPosition($position->x, $position->y, $position->z), $runtime_block_mapping->toRuntimeId($block->getFullId()), UpdateBlockPacket::FLAG_NETWORK, UpdateBlockPacket::DATA_LAYER_NORMAL), true);

			$tile = $world->getTileAt($position->x, $position->y, $position->z);
			if($tile instanceof Spawnable) {
				$network->sendDataPacket(BlockActorDataPacket::create(new BlockPosition($position->x, $position->y, $position->z), $tile->getSerializedSpawnCompound()), true);
			}
		}
	}
}