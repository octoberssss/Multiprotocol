...
final class PlayerNetwork {
	...
	private int $graphic_wait_duration = 0;
	...
	public function translateContainerOpen(PlayerSession $session, int $window_id, int $window_type, int $x, int $y, int $z): bool {
		...
		if(
			...
			$this->session->sendDataPacket(ContainerOpenPacket::blockInv($window_id, $current_menu->getType()->getWindowType(), BlockPosition::fromVector3($pos)));
			return true;
		}
		return false;
	}
