...
class PlayerSession {
	...
	public function setCurrentMenu(?InvMenu $menu, ?Closure $callback = null): void {
		...
		if($this->current_menu !== null) {
			$cb = function(bool $success) use ($callback) : void{
				if($this->current_menu !== null){
			...
			$this->network->waitUntil($this->network->getGraphicWaitDuration(), $cb);
/*			if($menu->getType() instanceof DoubleBlockActorMenuMetadata){
				$this->network->waitUntil($this->network->getGraphicWaitDuration(), $cb);
			} else {
				$cb(true); // don't wait if not needed
			}*/
		} else {
